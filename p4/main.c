...
read_write(const uint64_t N, const uint64_t K, const uint64_t V)
{
	...
	for (i=0; i<N; ++i) {
		...
		// 	kvdb_insert(kvdb, key, key_len, val, val_len) ||
...
static int
read_write_restore(const uint64_t N, const uint64_t K, const uint64_t V)
{
	uint64_t i, j, key_len, val_len, val_len_;
	void *key, *val, *val_;
	struct kvdb *kvdb;
	...
	/* restore */
	if (!(kvdb = kvdb_open_with_restore(PATHNAME))) {
		TRACE(0);
		return -1;
	}
...
static int read_write_large_restore(void)  { return read_write_restore(1234, 1234, 1234); }
static int read_write_small_restore(void)  { return read_write_restore(1234,  123,  123); }
static int read_write_single_restore(void) { return read_write_restore(   1,   23,   23); }
...
heavy_rewrite_restore(void)
{
	...
	// only 1 item, updated n times, old n-1 will be waste
	if ((1 != kvdb_size(kvdb)) || ((n-1) != kvdb_waste(kvdb))) {
		kvdb_close(kvdb);
		TRACE("software");
		return -1;
	}

	// only the last val is valid, no need to lookup N times

	if (kvdb_lookup(kvdb, KEY, SLEN(KEY), val_, &val_len_)) {
		debugf("HR RESTORE 1\n");
		exit(2);
	}
	if ((SLEN(val) != val_len_)) {
		debugf("HR RESTORE 2 expected[%d], got[%d] val[%s] i[%d]\n", SLEN(val), val_len_, val, i);
		exit(2);
	}
	if (memcmp(val, val_, val_len_)) {
		debugf("HR RESTORE 3\n");
...
basic_restore(void)
{
	...
	int v = 0;
	if ((v = kvdb_waste(kvdb))!=0) {
		debugf("FAILED 5 got v[%d]\n", v);
		kvdb_close(kvdb);
		exit(0);
	}

	debugf("-- BASIC RESTORE DONE --\n");
	...
	if (kvdb_update(kvdb, KEY, SLEN(KEY), VAL3, SLEN(VAL3)) ||
		// size = 1
		// waste = 2
		// key is already present, so this is like a replace, so the old version gets invalidated
	...
	debugf("\n\n-- BASIC 22 RESTORE DONE --\n\n");

	val_len = sizeof (val);

	
	if (+1 != kvdb_lookup(kvdb, KEY, SLEN(KEY), val, &val_len)) {
		debugf("No 1\n");
		kvdb_close(kvdb);
		exit(1);
	}
	...
	v = 0;
	if ((v = kvdb_size(kvdb)!=0)) {
		debugf("No 2 got size[%d]\n", v);
		kvdb_close(kvdb);
		exit(1);
	}

	if (3 != kvdb_waste(kvdb)) {
		debugf("No 3\n");
		kvdb_close(kvdb);
		exit(1);
	}
...
main(int argc, char *argv[])
{
	...
	TEST(basic_restore, "basic_restore");
	TEST(heavy_rewrite_restore, "heavy_rewrite_restore");
