...
void logfs_set_tail_offset(struct logfs *lfs, uint64_t offset);
void logfs_wb_add(struct logfs *lfs, void *buf, size_t buf_len);
void logfs_wb_deq(struct logfs *lfs);
size_t logfs_wb_avail(struct logfs *lfs);
...
int logfs_read_with_flush(struct logfs *logfs, void *buf, uint64_t off, size_t len, int do_flush);
