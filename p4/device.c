...
device_read(struct device *device, void *buf, uint64_t off, uint64_t len)
{
	...
	debugf("device_read: off[%ld] len[%ld]\n", (off_t) off,  (size_t) len);
...
device_write(struct device *device,
	     ...
	     uint64_t len)
{
	debugf("device_write: off[%ld] len[%ld]\n", (off_t) off,  (size_t) len);

	if (!( !len || buf )) {
		debugf("device_write: error 1\n");
		exit(0);
	}
	if (!( 0 == (off % device->block) )) {
		debugf("device_write: error 2\n");
		exit(0);
	}
	if (!( 0 == (len % device->block) )) {
		debugf("device_write: error 3\n");
		exit(0);
	}
	if (!( (off + len) <= device->size )) {
		debugf("device_write: error 4\n");
		exit(0);
	}
